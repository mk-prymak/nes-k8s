...
def main() -> None:
    parser = argparse.ArgumentParser(description="Pulse sensor simulator (single device)")
    ...
    parser.add_argument("--device-id", required=True, help="Stable device id, e.g. pulse-icu-a-bed-3")
    ...
    parser.add_argument("--bpm-min", type=int, default=55)
    parser.add_argument("--bpm-max", type=int, default=120)
    ...
    schema = load_schema("pulse.schema.json")
    ...
    try:
        while True:
            ...
            payload["bpm"] = rng.randint(args.bpm_min, args.bpm_max)
